...
BINSAVED="qemu gstr"
SHARESAVED="qemu hwdata icons osinfo certificates p11 gstr"
lib_audio_keywords="alsa jack pipewire pulse gstr"
lib_browser_launcher="gio-launch-desktop libasound.so libatk-bridge libatspi libcloudproviders libdb- libdl.so libedit libepoxy libgtk-3.so.0 libjson-glib libnssutil libpthread.so librt.so libtinysparql libwayland-cursor libX11-xcb.so libxapp-gtk3-module.so libXcursor libXdamage libXi.so libxkbfile.so libXrandr p11 pk"
LIBSAVED="gdk librsvg libdav libjson libtinfo libgiognutls qemu vte-urlencode-cwd libpxbackend \
libunwind.so liborc libnss libpcsclite.so libwoff libgpg-error.so libEGL.so libGLX.so libsharpyuv.so libjxl_cms.so \
libhwy.so libbrotlienc.so libyuv.so librav libSvtAv libaom.so libtss libevdev.so libexpat.so libnghttp libGLdispatch.so \
libogg.so libvorbisenc.so libFLAC.so libmpg123.so libmp3lame.so libxkbcommon libv4lconvert.so libtheora.so libgupnp-igd \
libslang.so libgpm.so libGL.so libGLU.so libglut.so pixbuf svg $lib_audio_keywords $lib_browser_launcher"
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (qemu-base|qemu-desktop|libvirt|virtiofsd|gstreamer|hicolor-icon-theme|xapp) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
...
_remove_more_bloatwares() {
	...
	lib_remove="gcc libgphobos.so perl systemd"
	for r in $lib_remove; do
		rm -Rf ./"$APP".AppDir/.junest/usr/lib/"$r"*
	done
	share_remove="gcc gir ibus/dicts/emoji icons/Adwaita/cursors/ icons/AdwaitaLegacy perl"
